...
from vllm.transformers_utils.configs.rwkv7 import (
    get_rwkv7_hybrid_attention_spec,
)
...
class RWKV7HybridAttention(nn.Module):
    def __init__(
        self,
        config,
        layer_idx: int,
        ...
        super().__init__()
        spec = get_rwkv7_hybrid_attention_spec(config.attn, layer_idx)
        if spec is None:
            raise ValueError(f"Layer {layer_idx} has no hybrid attention spec.")
        self.hidden_size = config.hidden_size
        self.total_num_heads = spec["num_heads"]
        self.total_num_kv_heads = spec["num_kv_heads"]
        self.tp_size = get_tp_world_size()
        if self.total_num_heads % self.tp_size != 0:
            raise ValueError("Hybrid attention heads must be divisible by TP size.")
        if self.total_num_kv_heads >= self.tp_size:
            if self.total_num_kv_heads % self.tp_size != 0:
                raise ValueError("Hybrid KV heads must be divisible by TP size.")
        elif self.tp_size % self.total_num_kv_heads != 0:
            raise ValueError("Hybrid KV heads must divide TP size when replicated.")
        ...
        self.attn = Attention(
            ...
            quant_config=quant_config,
            per_layer_sliding_window=spec["window_size"],
            ...
            for name, tensor in weights:
                for source_name, shard_id in (
                    ...
                    if name == source_name or name.startswith(source_name + "."):
                        name = "qkv_proj" + name[len(source_name) :]
...
class RWKV7Block(nn.Module, MambaBase):
    def __init__(
        ...
        self.max_local_value_dim = max(config.value_dim) // get_tp_world_size()
    ...
    def _forward_runtime(
        ...
        def store_v_first(
            ...
            destination[..., : self.local_value_dim].copy_(source)
        ...
        if attn_metadata is None:
            out, vf_out, _, _, _ = self._run_sequence(
                hidden_states, logical_v_first, None, None, None
            )
        ...
        if logical_v_first is not None:
            logical_v_first = logical_v_first[:num_actual_tokens]

        output_slice = output[:num_actual_tokens]
        v_first_slice = v_first_out[:num_actual_tokens]
        input_v_first_slice = None if v_first is None else v_first[:num_actual_tokens]
        if input_v_first_slice is None:
            v_first_slice.zero_()
        else:
            v_first_slice.copy_(input_v_first_slice)
        state_indices = attn_metadata.state_indices_tensor

        if attn_metadata.num_decode_tokens > 0:
            # Full CUDA graphs pad the request batch with zero-length decode
            # requests. ``num_decodes`` includes those padded requests, while
            # ``num_decode_tokens`` only counts real one-token decodes. Avoid
            # indexing the state cache with the padded ``PAD_SLOT_ID`` entries.
            ...
            out, vf_out, attn_shift, recurrent, ffn_shift = self._run_decode_batch(
                ...
                (
                    None
                    if logical_v_first is None
                    else logical_v_first[: attn_metadata.num_decode_tokens]
                ),
                *states,
            )
            output_slice[: attn_metadata.num_decode_tokens] = out
            v_first_slice[: attn_metadata.num_decode_tokens, : self.local_value_dim] = (
                vf_out
            )
        ...
        if attn_metadata.num_prefills > 0:
            ...
            out, vf_out, attn_shift, recurrent, ffn_shift = self._run_prefill_batch(
                ...
                (
                    None
                    if logical_v_first is None
                    else logical_v_first[prefill_token_offset:num_actual_tokens]
                ),
                prefill_query_start_loc,
                *states,
            )
    ...
    def forward(
        ...
        v_first_out = torch.empty(
            (hidden_states.shape[0], self.max_local_value_dim),
...
class RWKV7HybridBlock(nn.Module, MambaBase):
    """Transformer attention plus RWKV token-shift FFN state."""
    ...
    def __init__(
        ...
        self.num_heads = config.num_heads
        self.head_dim = config.head_dim
        self.max_value_dim = max(config.value_dim)
        self.max_local_value_dim = max(config.value_dim) // get_tp_world_size()
        ...
        self.attn = RWKV7HybridAttention(
            config=config,
            layer_idx=layer_idx,
            cache_config=cache_config,
            quant_config=quant_config,
        ...
        if prefix in compilation_config.static_forward_context:
            raise ValueError(f"Duplicate layer name: {prefix}")
        compilation_config.static_forward_context[prefix] = self
        # HybridAttentionMambaModelConfig requires every linear-attention layer
        # to expose the same state tuple.  Attention layers only consume the
        # FFN shift state, but retain the two RWKV attention slots so cache
        # allocation/copying stays uniform across mixed layer types.
        self.kv_cache = (
            torch.tensor([]),
            torch.tensor([]),
            torch.tensor([]),
    ...
    def get_state_shape(self) -> tuple[tuple[int, ...], ...]:
        return (
            ...
            (
                self.num_heads // get_tp_world_size(),
                self.max_value_dim // self.num_heads,
                self.head_dim,
            ),
            (self.hidden_size,),
        )
    ...
    def _get_ffn_states(
        self, slot_ids: torch.Tensor, has_initial_state: torch.Tensor | None = None
    ) -> torch.Tensor:
        states = self.kv_cache[2].index_select(0, slot_ids)
        if has_initial_state is None:
            return states
        return torch.where(
            has_initial_state.to(states.device)[:, None],
            states,
            torch.zeros_like(states),
        )

    @torch.compiler.disable
    def _store_ffn_states(self, slot_ids: torch.Tensor, states: torch.Tensor) -> None:
    ...
    def _run_ffn(
        ...
        if attn_metadata.num_decode_tokens > 0:
            ...
            decode_states = self._get_ffn_states(decode_slot_ids)
            decode_output, decode_states = self.ffn.forward_decode_batch(
                hidden_states[: attn_metadata.num_decode_tokens], decode_states
            )
            output[: attn_metadata.num_decode_tokens] = decode_output
            self._store_ffn_states(decode_slot_ids, decode_states)
        ...
        if attn_metadata.num_prefills > 0:
            ...
            seq_lens = attn_metadata.seq_lens[prefill_req_offset:prefill_req_end]
            prefill_states = self._get_ffn_states(
                prefill_slot_ids, seq_lens > query_lens
            )
    ...
    def forward(
        ...
        if self.pre_norm is not None:
            residual = self.pre_norm(residual)
        attn_input = self.attn_norm(residual)
        hidden_states = residual + self.attn(positions, attn_input)
        ffn_input = self.ffn_norm(hidden_states)
        hidden_states = hidden_states + self._run_ffn(ffn_input, attn_metadata)
        if v_first is None:
...
class RWKV7Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.local_value_dim = max(value_dims) // get_tp_world_size()
        ...
        def make_layer(prefix: str) -> nn.Module:
            ...
            layer_class = (
                RWKV7HybridBlock
                if get_rwkv7_hybrid_attention_spec(config.attn, layer_idx) is not None
                else RWKV7Block
            )
            return layer_class(
                config=config,
    ...
    def get_mamba_state_shape_from_config(
        ...
        return (
            ...
            (
                ...
                config.head_dim,
                max(config.value_dim) // config.num_heads,
            ),
            (config.hidden_size,),
        )
...
class RWKV7HybridForCausalLM(RWKV7ForCausalLM, IsHybrid):
    """RWKV7 architecture marker for checkpoints containing full attention."""
